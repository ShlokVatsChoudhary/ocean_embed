...
def test_service_profile_contract():
    ...
    response = get_profile(10.0, 20.0, date(2024, 1, 1), model=model)
...
def test_profile_model_mapping_and_validation():
    model = FakeProfileModel()
    response = get_profile(12.5, 21.5, date(2024, 2, 3), model=model)

    assert model.received_request.latitude == 12.5
    assert model.received_request.longitude == 21.5
    assert model.received_request.date == date(2024, 2, 3)
    ...
    filtered = get_profile(12.5, 21.5, date(2024, 2, 3), model=model, depth=125.0)
    assert len(filtered.profile) == 1
    assert filtered.profile[0].depth == 125.0
    assert filtered.profile[0].temperature == 8.0

    try:
        get_profile(12.5, 21.5, date(2024, 2, 3), model=model, depth=15.0)
...
def test_profile_requires_exactly_15_model_outputs():
    for values in ([float(value) for value in range(14)], [float(value) for value in range(16)]):
        ...
        try:
            get_profile(10.0, 20.0, date(2024, 1, 1), model=model)
...
def test_profile_route_response_is_valid():
    ...
    try:
        response = client.get("/api/profile?latitude=10&longitude=20&date=2024-01-01")
...
def test_comparison_difference_calculation():
    ...
    response = get_comparison(10.0, 20.0, date(2024, 1, 2), 100.0, model=model, glorys_source=glorys)
    ...
    assert model.received_request.date == date(2024, 1, 2)
    ...
    assert glorys.received_request["target_date"] == date(2024, 1, 2)
    assert glorys.received_request["depth"] == 100.0


def test_comparison_handles_signed_and_zero_differences():
    positive = get_comparison(1.0, 2.0, date(2024, 1, 3), 50.0, model=FakeComparisonModel(10.0), glorys_source=FakeGlorysSource(7.0))
    negative = get_comparison(1.0, 2.0, date(2024, 1, 3), 50.0, model=FakeComparisonModel(7.0), glorys_source=FakeGlorysSource(10.0))
    zero = get_comparison(1.0, 2.0, date(2024, 1, 3), 50.0, model=FakeComparisonModel(8.0), glorys_source=FakeGlorysSource(8.0))
...
def test_comparison_route_uses_dependency_injection():
    ...
    try:
        response = client.get("/api/comparison?latitude=11&longitude=22&date=2024-01-05&depth=75")
        assert response.status_code == 200
        payload = response.json()
